...
const getCashReceiptRetentionBreakdown = (receipt = {}) => {
    const retentionIr2 = safeNumber(receipt.retentionIr2 ?? receipt.retencionIr2);
    const retentionMunicipal1 = safeNumber(receipt.retentionMunicipal1 ?? receipt.retencionMunicipal1);
    const detailedTotal = safeNumber(retentionIr2 + retentionMunicipal1);
    const storedTotal = safeNumber(receipt.retentionTotal ?? receipt.retencionTotal);

    return {
        retentionIr2,
        retentionMunicipal1,
        // The detailed fiscal fields are authoritative when present. This repairs
        // legacy receipts whose cached total was saved as zero or became stale.
        retentionTotal: detailedTotal > 0 ? detailedTotal : storedTotal,
    };
};

const getCashReceiptRetentionTotal = (receipt = {}) => (
    getCashReceiptRetentionBreakdown(receipt).retentionTotal
);

const isCashReceiptIncludedInSicarRecovery = (receipt = {}) => (
    receipt.includedInSicarRecovery !== false
    && normalizeText(receipt.sicarRecoveryMode) !== 'EXTERNAL_ACCOUNTING'
);

const getExternalCashReceiptTotal = (receipts = []) => safeNumber(
    (Array.isArray(receipts) ? receipts : []).reduce((sum, receipt) => (
        sum + (isCashReceiptIncludedInSicarRecovery(receipt) ? 0 : safeNumber(receipt.amount || receipt.cantidad))
    ), 0)
);
...
const normalizeCashReceiptRecord = (item = {}) => {
    ...
    const { retentionIr2, retentionMunicipal1, retentionTotal } = getCashReceiptRetentionBreakdown(item);
    return {
        ...item,
        ...
        includedInSicarRecovery: isCashReceiptIncludedInSicarRecovery(item),
...
const getCashClosureComparableExpectedTotal = (sicarExpected = 0, externalCreditRecoveryTotal = 0) => (
    safeNumber(safeNumber(sicarExpected) + safeNumber(externalCreditRecoveryTotal))
);
...
const normalizeCashClosureStoredTotals = (closure = {}) => {
    ...
    const externalCreditRecoveryTotal = safeNumber(
        closure.externalCreditRecoveryTotal ?? getExternalCashReceiptTotal(getCashClosureReceipts(closure))
    );
    const expectedAfterRetentions = getCashClosureComparableExpectedTotal(sicarExpected, externalCreditRecoveryTotal);
    const manualTotalWithRetentions = getCashClosureManualTotalWithRetentions(manualTotal, retentionAdjustment);
    const difference = getCashClosureDifference(manualTotal, expectedAfterRetentions, retentionAdjustment);
    return {
        manualTotal,
        ...
        comparisonExpectedTotal: expectedAfterRetentions,
...
const getClosurePaymentRoute = (method = '') => {
    ...
    if (normalized.includes('POS') || normalized.includes('TARJETA')) {
        if (normalized.includes('BANPRO')) return { type: 'pos', key: 'banpro' };
        if (normalized.includes('LAFISE')) return { type: 'pos', key: 'lafise' };
        if (normalized.includes('BAC')) return { type: 'pos', key: 'bac' };
        return null;
    }
    if (!normalized.includes('TRANSFER')) return null;
    if (normalized.includes('LAFISE') && (compact.includes('USD') || normalized.includes('DOLAR'))) {
        return { type: 'transfer', key: 'lafiseUsd', currency: 'USD' };
    }
    if (normalized.includes('BAC') && (compact.includes('USD') || normalized.includes('DOLAR'))) {
        return { type: 'transfer', key: 'bacUsd', currency: 'USD' };
    }
    if (compact.includes('BAC2')) return { type: 'transfer', key: 'bac2', currency: 'NIO' };
    if (normalized.includes('BANPRO')) return { type: 'transfer', key: 'banpro', currency: 'NIO' };
    if (normalized.includes('LAFISE')) return { type: 'transfer', key: 'lafise', currency: 'NIO' };
    if (normalized.includes('BAC')) return { type: 'transfer', key: 'bac', currency: 'NIO' };
    return null;
};

const buildClosureDocumentPaymentAutomation = ({
    invoices = [],
    cashReceipts = [],
    closureId = '',
} = {}) => {
    ...
    const appendDocumentPayments = ({
        ...
        const documentPrefix = sourceType === 'receipt' ? 'R' : 'F';
        const documentReference = `${documentPrefix}-${sourceNumber || 'SIN NUMERO'}`;

        (Array.isArray(rows) ? rows : []).forEach((row, rowIndex) => {
            const route = getClosurePaymentRoute(row.method);
            const amountCordobas = safeNumber(row.amount);
            if (!route || amountCordobas <= 0) return;

            const rowIdentity = row.id || row.localId || rowIndex;
            const autoSourceKey = [
                closureId || 'closure',
                sourceType,
                sourceId || sourceNumber || 'document',
                rowIdentity,
                route.type,
                route.key,
            ].join(':');
            const commonFields = {
                localId: `auto_${autoSourceKey.replace(/[^a-z0-9]+/gi, '_')}`,
                autoGenerated: true,
                autoEdited: false,
                autoSourceKey,
                ...
                sourceDocumentNumber: String(sourceNumber || ''),
                sourceDocumentLabel: documentReference,
                sourcePaymentMethod: row.method || '',
                amountCordobas,
            };
            ...
            const exchangeRate = route.currency === 'USD' ? TRANSFER_USD_EXCHANGE_RATE : 1;
            transferDetails[route.key].push({
                ...commonFields,
                clientName: customerName || '',
                amount: route.currency === 'USD' ? safeNumber(amountCordobas / exchangeRate) : amountCordobas,
                reference: `${documentReference}${row.reference ? ` / ${row.reference}` : ''}`,
                currency: route.currency || 'NIO',
                exchangeRate,
            });
        });
    };

    invoices.forEach((invoice, invoiceIndex) => appendDocumentPayments({
        sourceType: 'invoice',
        sourceId: invoice.docId || invoice.id || `invoice-${invoiceIndex}`,
        sourceNumber: invoice.invoiceNumber || invoice.numeroFactura || '',
        customerName: invoice.customerName || invoice.cliente || '',
        rows: getInvoicePaymentRows(invoice),
    }));

    cashReceipts.forEach((receipt, receiptIndex) => appendDocumentPayments({
        sourceType: 'receipt',
        sourceId: receipt.docId || receipt.id || `receipt-${receiptIndex}`,
        sourceNumber: receipt.receiptNumber || receipt.numeroRecibo || '',
        customerName: receipt.customerName || receipt.recibiDe || receipt.cliente || '',
        rows: [{
            id: `receipt-payment-${receipt.docId || receipt.id || receiptIndex}`,
            method: receipt.paymentMethod || receipt.metodoPago || '',
            amount: getCashReceiptNetAmount(receipt),
            reference: receipt.reference || receipt.referencia || '',
        }],
    }));
...
const mergeAutomaticDetailRows = (currentRows = [], generatedRows = []) => {
    const current = Array.isArray(currentRows) ? currentRows : [];
    const consumedIndexes = new Set();
    const findExistingRow = (generated) => {
        const exactIndex = current.findIndex((row, index) => (
            !consumedIndexes.has(index)
            && row.autoSourceKey
            && row.autoSourceKey === generated.autoSourceKey
        ));
        if (exactIndex >= 0) return exactIndex;

        const sourceNumber = normalizeText(generated.sourceDocumentNumber);
        return current.findIndex((row, index) => {
            if (consumedIndexes.has(index) || row.autoGenerated || row.autoSourceKey) return false;
            const sameAmount = Math.abs(safeNumber(row.amount) - safeNumber(generated.amount)) <= 0.05;
            if (!sameAmount) return false;
            const rowText = normalizeText(`${row.reference || ''} ${row.description || ''}`);
            if (sourceNumber && rowText.includes(sourceNumber)) return true;
            const sameClient = normalizeText(row.clientName) && normalizeText(row.clientName) === normalizeText(generated.clientName);
            return Boolean(sameClient);
        });
    };

    const automaticRows = generatedRows.map((generated) => {
        const existingIndex = findExistingRow(generated);
        if (existingIndex < 0) return generated;
        consumedIndexes.add(existingIndex);
        const existing = current[existingIndex];
        if (existing.autoEdited || !existing.autoGenerated) {
            return {
                ...generated,
                ...existing,
                localId: existing.localId || generated.localId,
                autoGenerated: true,
                autoEdited: true,
                autoSourceKey: generated.autoSourceKey,
            };
        }
        return { ...existing, ...generated, localId: existing.localId || generated.localId };
    });
    const manualRows = current.filter((row, index) => (
        !consumedIndexes.has(index)
        && !row.autoGenerated
        && !row.autoSourceKey
    ));
...
const buildClosureAccountingSummary = ({
    ...
    const comparableCreditRecoveryTotal = getCashClosureComparableExpectedTotal(creditRecoveryTotal, externalCreditRecoveryTotal);
    const ticketCashReceipts = safeNumber(comparableCreditRecoveryTotal - cashReceiptGrossTotal);
    ...
    return {
        general: {
            ...
            sicarCreditRecoveryTotal: safeNumber(creditRecoveryTotal),
            externalCreditRecoveryTotal: safeNumber(externalCreditRecoveryTotal),
            creditRecoveryTotal: comparableCreditRecoveryTotal,
        },
        stampedDocuments: {
...
const syncLinkedClosureForCashReceipt = async (receiptId = '', receiptPayload = {}) => {
    ...
    const externalCreditRecoveryTotal = getExternalCashReceiptTotal(receiptsForTotals);
    ...
    const accountingSummary = closure.accountingSummary ? {
        ...
        sicarTickets: {
            ...
            cashReceiptTickets: safeNumber(comparisonExpectedTotal - cashReceiptGrossTotal),
        },
        general: {
            ...(closure.accountingSummary.general || {}),
            sicarCreditRecoveryTotal: sicarExpected,
            externalCreditRecoveryTotal,
            creditRecoveryTotal: comparisonExpectedTotal,
        },
        paymentBreakdown: {
    ...
    await setDoc(closureRef, {
        ...
        retentionAdjustment,
        cashReceiptRetentionTotal,
        externalCreditRecoveryTotal,
        comparisonExpectedTotal,
        expectedAfterRetentions,
...
const syncLinkedClosureForStampedInvoice = async (invoiceId = '', invoicePayload = {}) => {
    ...
    const retentionAdjustment = safeNumber(invoiceRetentionTotal + cashReceiptRetentionTotal);
    const externalCreditRecoveryTotal = getExternalCashReceiptTotal(cashReceipts);
    const sicarExpected = safeNumber(closure.sicarExpected);
    const manualTotal = safeNumber(closure.manualTotal);
    const comparisonExpectedTotal = getCashClosureComparableExpectedTotal(sicarExpected, externalCreditRecoveryTotal);
    const expectedAfterRetentions = comparisonExpectedTotal;
...
const getSicarPaymentMethodForAccounting = (value = '') => {
    ...
    if (normalized.includes('TRANSFER')) {
        if (normalized.includes('LAFISE') && (compact.includes('USD') || normalized.includes('DOLAR'))) return 'TRANSFERENCIA LAFISE USD';
        if (normalized.includes('BAC') && (compact.includes('USD') || normalized.includes('DOLAR'))) return 'TRANSFERENCIA BAC USD';
        if (compact.includes('BAC2')) return 'TRANSFERENCIA BAC (2)';
                    ...
                    return (
                        <div key={row.localId || index} className="grid gap-2 rounded-2xl border border-slate-200 bg-white p-3 md:grid-cols-[1.7fr_0.8fr_auto]">
                            <div>
                                ...
                                />
                                {row.autoGenerated && (
                                    <div className="mt-1 text-[10px] font-black uppercase tracking-[0.14em] text-amber-700">
                                        Autocargado desde {row.sourceDocumentLabel || 'documento membretado'} · editable
                                    </div>
                                )}
                            </div>
                            <input
                ...
                <div key={row.localId || index} className="grid gap-2 rounded-2xl border border-slate-200 bg-white p-3 md:grid-cols-[1.4fr_1fr_1fr_auto]">
                    {type === 'transfer' ? (
                        <div>
                            ...
                            {row.autoGenerated && (
                                <div className="mt-1 text-[10px] font-black uppercase tracking-[0.14em] text-sky-700">
                                    Autocargado desde {row.sourceDocumentLabel || 'documento membretado'} · editable
                                </div>
                            )}
                        </div>
...
const PosSettlementComparison = ({ expected = 0, settled = 0 }) => {
    const difference = safeNumber(settled - expected);
    const isBalanced = Math.abs(difference) <= 0.05;
    const statusLabel = isBalanced
        ? 'Liquidaciones cuadradas'
        : difference < 0
            ? `Faltan liquidaciones por ${fmt(Math.abs(difference))}`
            : `Sobran liquidaciones por ${fmt(difference)}`;

    return (
        <div className={`mt-3 rounded-2xl border p-3 ${isBalanced ? 'border-emerald-200 bg-emerald-50' : 'border-amber-200 bg-amber-50'}`}>
            <div className="grid gap-2 sm:grid-cols-3">
                <div className="rounded-xl bg-white px-3 py-2">
                    <div className="text-[9px] font-black uppercase tracking-[0.16em] text-slate-400">Membretadas / recibos</div>
                    <div className="mt-1 font-mono text-sm font-black text-slate-950">{fmt(expected)}</div>
                </div>
                <div className="rounded-xl bg-white px-3 py-2">
                    <div className="text-[9px] font-black uppercase tracking-[0.16em] text-slate-400">Liquidaciones</div>
                    <div className="mt-1 font-mono text-sm font-black text-slate-950">{fmt(settled)}</div>
                </div>
                <div className="rounded-xl bg-white px-3 py-2">
                    <div className="text-[9px] font-black uppercase tracking-[0.16em] text-slate-400">Diferencia</div>
                    <div className={`mt-1 font-mono text-sm font-black ${isBalanced ? 'text-emerald-700' : 'text-amber-700'}`}>{fmt(difference)}</div>
                </div>
            </div>
            <div className={`mt-2 text-xs font-black ${isBalanced ? 'text-emerald-700' : 'text-amber-800'}`}>{statusLabel}</div>
        </div>
    );
};
...
function CashClosure({ data, branchContext }) {
    ...
    const closurePaymentAutomation = useMemo(() => buildClosureDocumentPaymentAutomation({
        invoices: closureInvoices,
        cashReceipts: closureCashReceipts,
        closureId: selectedClosureId,
    }), [closureCashReceipts, closureInvoices, selectedClosureId]);

    useEffect(() => {
        setTransfers((current) => Object.fromEntries(TRANSFER_BANKS.map(({ key }) => [
            key,
            mergeAutomaticDetailRows(
                current[key] || [],
                selectedClosureId ? closurePaymentAutomation.transferDetails[key] || [] : []
            ),
        ])));
        setHouseDiscountDetails((current) => mergeAutomaticDetailRows(
            current,
    ...
    const externalCreditRecoveryTotal = getExternalCashReceiptTotal(closureCashReceipts);
    ...
    const loadWaitingClosure = (closure) => {
        ...
        const loadedReceipts = (closure.cashReceiptDrafts || closure.cashReceipts || []).map((receipt) => {
            const receiptId = receipt.docId || receipt.id || receipt.receiptId || '';
            const latestReceipt = latestReceiptById.get(receiptId);
            return createCashReceiptDraft(latestReceipt ? { ...receipt, ...latestReceipt } : receipt, closure.date || todayString());
        });
    ...
    const updateTransfer = (bank, index, field, value) => {
        setTransfers((prev) => ({
            ...prev,
            [bank]: (prev[bank] || []).map((row, rowIndex) => (rowIndex === index ? {
                ...row,
                [field]: value,
        ...
        try {
            ...
            const payload = {
                ...
                cashReceipts: isWaiting ? [] : savedCashReceipts.map((receipt) => ({
                    ...
                    includedInSicarRecovery: isCashReceiptIncludedInSicarRecovery(receipt),
                    sicarRecoveryMode: receipt.sicarRecoveryMode || (isCashReceiptIncludedInSicarRecovery(receipt) ? 'included' : 'external_accounting'),
    ...
    return (
        ...
        <div className="grid gap-5 xl:grid-cols-[0.9fr_1.3fr]">
            <div className="space-y-5">
                ...
                <Section title="Transferencias por cliente" eyebrow="Autocarga F/R · siempre editable">
                ...
                <Section title="Liquidaciones POS" eyebrow="Comparativo contra documentos membretados">
                    <div className="grid gap-4">
                        {POS_BANKS.map((bank) => (
                            <div key={bank.key}>
                                <DetailRows
                                    title={`POS ${bank.label} · liquidado ${fmt(posTotals[bank.key])}`}
                                ...
                                />
                                <PosSettlementComparison
                                    expected={closurePaymentAutomation.posExpectedTotals[bank.key] || 0}
                                    settled={posTotals[bank.key] || 0}
                                />
                            </div>
                        ))}
                    </div>
                ...
                <Section title="Descuentos de la casa" eyebrow="Autocarga desde facturas y recibos">
                    <DetailRows
            ...
            <div className="space-y-5">
                <Section title="Cierre de caja" eyebrow="Formulario operativo" action={<Badge tone={shouldTrackDifference ? 'red' : 'green'}>{shouldTrackDifference ? 'Con diferencia' : 'Cuadrado'}</Badge>}>
                    ...
                    <div className="mt-5 grid gap-3 md:grid-cols-4">
                        <SummaryCard
                            label={externalCreditRecoveryTotal > 0 ? 'Esperado conciliable' : 'SICAR esperado'}
                            value={fmt(comparisonExpectedTotal)}
                            tone="blue"
                        />
                    ...
                    {externalCreditRecoveryTotal > 0 && (
                        <div className="mt-3 rounded-2xl border border-sky-200 bg-sky-50 px-4 py-3 text-sm font-bold text-sky-900">
                            Incluye {fmt(externalCreditRecoveryTotal)} de recibos creados en Cuentas por Cobrar que no fueron registrados en SICAR. Sus retenciones tambien forman parte de la conciliacion.
                        </div>
                    )}
                ...
                <Section title="Facturas membretadas del cierre" eyebrow="Retenciones que reducen caja">
                    ...
                    <div className="mt-6 rounded-3xl border border-amber-200 bg-amber-50/30 p-4">
                        <div className="mb-4 flex flex-col gap-3 xl:flex-row xl:items-center xl:justify-between">
                            ...
                            <div className="flex flex-wrap items-center gap-2">
                                <Badge tone="amber">Bruto {fmt(closureCashReceipts.reduce((sum, receipt) => safeNumber(sum + safeNumber(receipt.amount)), 0))}</Badge>
                                <Badge tone="blue">Ret. {fmt(cashReceiptRetentionTotal)}</Badge>
                            </div>
                        </div>
                                    ...
                                    return (
                                        <div key={receiptId} className="flex flex-col gap-2 rounded-2xl border border-amber-200 bg-white px-4 py-3 sm:flex-row sm:items-center sm:justify-between">
                                            <div className="min-w-0">
                                                ...
                                                {!isCashReceiptIncludedInSicarRecovery(receipt) && (
                                                    <div className="mt-1 text-[10px] font-black uppercase tracking-[0.14em] text-sky-700">Abono solo contable - se suma al esperado</div>
                                                )}
                                            </div>
                                            <div className="flex items-center gap-2">
...
const calculateClosureEditTotals = (form = {}) => {
    ...
    const externalCreditRecoveryTotal = safeNumber(form.externalCreditRecoveryTotal);
    const expectedAfterRetentions = getCashClosureComparableExpectedTotal(sicarExpected, externalCreditRecoveryTotal);
    const manualTotalWithRetentions = getCashClosureManualTotalWithRetentions(manualTotal, retentionAdjustment);
    const difference = getCashClosureDifference(manualTotal, expectedAfterRetentions, retentionAdjustment);
    const shouldTrackDifference = Math.abs(difference) > CASH_DIFFERENCE_THRESHOLD;
    ...
    const saveEditedClosure = async () => {
        ...
        try {
            ...
            batch.set(doc(db, 'cierres_caja', editForm.id), sanitizeFirestoreData({
                ...
                sicarExpected: totals.sicarExpected,
                externalCreditRecoveryTotal: totals.externalCreditRecoveryTotal,
                comparisonExpectedTotal: totals.comparisonExpectedTotal,
                cashSalesTotal: safeNumber(editForm.cashSalesTotal),
                creditSalesTotal: safeNumber(editForm.creditSalesTotal),
                creditRecoveryTotal: safeNumber(editForm.creditRecoveryTotal),
                retentionAdjustment: totals.retentionAdjustment,
