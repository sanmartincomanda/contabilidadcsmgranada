...
import { EXPENSE_ACCOUNTS, EXPENSE_PAYMENT_OPTIONS } from '../services/expenseAccounting';
...
const EMPTY_FORM = {
    ...
    accountingAccountCode: '', accountingAccountName: '', metodoPago: '', referenciaPago: '', subtotal: '', iva: '', total: '',
                                ...
                                {!draft ? (
                                    ...
                                    <div className="space-y-5">
                                        <div className="grid gap-4 sm:grid-cols-2">
                                            <Field label="Tipo"><select className={controlClass} value={form.tipoRegistro} onChange={(event) => setForm((current) => ({ ...current, tipoRegistro: event.target.value, categoria: '', subcategoria: '', accountingAccountId: '', accountingAccountCode: '', accountingAccountName: '', metodoPago: '' }))}><option value="">Seleccionar</option><option value="gasto">Gasto operativo</option><option value="compra">Compra / costo</option></select></Field>
                                            ...
                                            {form.tipoRegistro === 'compra' && <Field label="Categoría / subcategoría" className="sm:col-span-2"><select className={controlClass} value={`${form.categoria}|||${form.subcategoria}`} onChange={(event) => { const option = CATEGORY_OPTIONS.find((item) => item.value === event.target.value); if (option) setForm((current) => ({ ...current, categoria: option.category, subcategoria: option.subcategory })); }}><option value="|||">Seleccionar</option>{CATEGORY_OPTIONS.filter((option) => option.category === 'Costos de venta / compras').map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}</select></Field>}
                                            <Field label={form.tipoRegistro === 'gasto' ? 'Cuenta hija del gasto' : 'Cuenta contable'} className="sm:col-span-2"><select className={controlClass} value={form.accountingAccountCode || form.accountingAccountId} onChange={(event) => { const account = form.tipoRegistro === 'gasto' ? EXPENSE_ACCOUNTS.find((item) => item.code === event.target.value) : accounts.find((item) => getAccountCode(item) === event.target.value); setForm((current) => ({ ...current, accountingAccountId: account?.id || event.target.value, accountingAccountCode: event.target.value, accountingAccountName: account?.name || getAccountName(account) })); }}><option value="">Seleccionar cuenta existente</option>{(form.tipoRegistro === 'gasto' ? EXPENSE_ACCOUNTS : accounts).map((account) => <option key={account.id || getAccountCode(account)} value={account.code || getAccountCode(account)}>{account.code || getAccountCode(account)} · {account.name || getAccountName(account)}</option>)}</select></Field>
                                            <Field label="Método de pago"><select className={controlClass} value={form.metodoPago} onChange={(event) => setForm((current) => ({ ...current, metodoPago: event.target.value }))}><option value="">Seleccionar</option>{(form.tipoRegistro === 'gasto' ? EXPENSE_PAYMENT_OPTIONS.map((option) => option.label) : PURCHASE_PAYMENT_METHODS).map((method) => <option key={method} value={method}>{method}</option>)}</select></Field>
                                            ...
                                            {String(form.metodoPago).toUpperCase().includes('CREDITO') && <Field label="Vencimiento"><input type="date" className={controlClass} value={form.vencimiento} onChange={(event) => setForm((current) => ({ ...current, vencimiento: event.target.value }))} /></Field>}
                                        </div>
