...
$taskAction = New-ScheduledTaskAction `
    -Execute 'wscript.exe' `
    -Argument "`"$hiddenRunnerPath`" `"runSicarStampedInvoiceWatcher.ps1`" -IntervalMs $safeInterval"
...
Register-ScheduledTask `
    ...
    -Description "Escucha facturas membretadas SICAR en MySQL cada $safeInterval ms y sube a Firebase solo cuando hay fac_id nuevo." `
    -Force | Out-Null

Write-Host "Tarea '$TaskName' creada correctamente. Intervalo interno: $safeInterval ms."
