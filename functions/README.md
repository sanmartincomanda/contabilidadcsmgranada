...
Para imprimir facturas membretadas desde la app, el servidor SICAR puede dejar corriendo un watcher local que revisa MySQL cada 15 segundos y solo escribe en Firebase cuando aparece una nueva fila en `factura`.
...
```powershell
cd functions
.\scripts\registerSicarStampedInvoiceWatcherTask.ps1 -IntervalMs 15000
...
```powershell
cd functions
.\scripts\registerSicarStampedInvoiceWatcherTask.ps1 -IntervalMs 15000 -StartupBackfillDays 7
Start-ScheduledTask -TaskName "SICAR Stamped Invoice Watcher"
```
